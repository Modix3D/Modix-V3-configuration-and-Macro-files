; Modix Big-120X, Generation 4, Single Printhead
...
M550 P"Big 120X V4"												; set printer name
...
M569 P6 S1													; Physical drive 6 . Z-B-L (0,600) 
M569 P7 S1													; Physical drive 7 . Z-F-L (0,0) 
M569 P8 S1													; Physical drive 8 . Z-F-R (1200,0) 
M569 P9 S1													; Physical drive 9 . Z-B-R (1200,600) 
...
M92 X80 Y80 Z2000 E418.5:418.5							; Set steps per mm
M566 X360 Y360 Z120 E3600:3600 P1							; Set maximum instantaneous speed changes (mm/min)
M203 X18000 Y9000 Z300 E12000:12000							; Set maximum speeds (mm/min) 			***normal Z max 200mm/min
M201 X3000 Y1000 Z120 E1000:1000							; Set accelerations (mm/s^2)
M204 P800													; Set print and travel accelerations  (mm/s^2)
M906 X2000 Y1800 E1000:1000 I50								; Set motor currents (mA) and motor idle factor in per cent
M906 Z2000 I50												; set Z motor currents 				*****normal 2000mA
...
; Endstops
M574 X1 S1 P"duex.e5stop + duex.e6stop"                            	; configure switch-type (e.g. microswitch) endstop for low end on X via pin xstop
M574 Y2 S1 P"duex.e4stop"                            				; configure switch-type (e.g. microswitch) endstop for low end on Y via pin ystop
...
M557 X{move.axes[0].min + sensors.probes[0].offsets[0] + 1, move.axes[0].max + sensors.probes[0].offsets[0] - 1} Y{move.axes[1].min + sensors.probes[0].offsets[1] + 1, move.axes[1].max + sensors.probes[0].offsets[1] - 1} P20:10
...
M950 J3 C"duex.e2stop" 									; create Input Pin 2 on pin E4 to for M581 Command.
